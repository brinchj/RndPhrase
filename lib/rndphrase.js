...
rndphrase.RndPhrase = {
    ...
    self_test : function () {
        ...
        for (var i = 0; i < tests.length; i++) {
            if (RNDPHRASE.build_generator(tests[i].seed)(tests[i].domain)(tests[i].passwd) !== tests[i].hash) {
                return false;
            }
        };
        return true;
    ...
    keypress: function (E) {
        ...
        if (String.fromCharCode(E.which) === '@' &&
            ...
            I.addEventListener('blur', _update, false); // TODO: Remove event handler
    ...
    keydown: function(E) {
        ...
        if (I._rndphrase !== undefined) {
            var must_update = [ 13, 9 ], // enter, tab
                delete_code = [ 8, 46 ]; // backspace, delete
            if (must_update.indexOf(E.which) !== -1) {

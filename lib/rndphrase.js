...
rndphrase.RndPhrase = {
    ...
    set_seed_hash : function(seed_hash) {
        ...
        RNDPHRASE.generator = RNDPHRASE.build_generator(seed_hash);
    },
    ...
    update_input : function(S,I) { return function(E) {
    ...
    keypress: function(S){ return function (E) {
            var I = E.target;
            if(!S.R.is_password(I)) return true;
            if(String.fromCharCode(E.which) == '@' &&
                (I.value == undefined || I.value.length == 0)) {
                // new rndphrase password field
                S.R.field_set_tag(I, "ready");
                var _update  = S.R.update_input(S, I);
    ...
    patch_document : function(host, doc) {
        ...
        var S = function() {
            ...
            this.G = _this.generator(host);
            return this;
        }();
        doc.addEventListener("keypress", S.A(this.keypress), true);
        doc.addEventListener("keydown",  S.A(this.keydown), true);
    }
};
